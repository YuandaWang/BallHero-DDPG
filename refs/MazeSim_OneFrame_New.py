...
class MazeSimulator(object):
    ...
    def step(self, dt):
        ...
        Also resolve collisions with the hero and reward
        Also trigger environment scroll
        """
    ...
    def explore_maze(self):
        posX = int(math.floor(self.hero.position[0]/300.0))
        posY = int(math.floor(self.hero.position[1]/250.0))
        if self.maze_grid[posX][posY] < 1:
            self.maze_grid[posX][posY] = 1
            return True
        else:
            return False
        
        
    def collect_reward(self, last_action, observation):
        ...
        if nearest_obstacle < self.settings['obstacle_tolerance']:
            ...
            obstacle_reward = -(1 - norm_distance)**2 * 10
        else:
            obstacle_reward = 0.0
    ...
    def generate_observation_lines(self):
        ...
        # This function just draw the lines, doesn't really measure the distance
        result = []
